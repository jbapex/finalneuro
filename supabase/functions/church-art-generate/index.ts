  ...
  const content: unknown =
    hasSubject || hasStyle || hasLogo
      ? [
          ...
          ...subjectImageUrls.slice(0, 6).map((subjectUrl) => ({ type: "image_url" as const, image_url: { url: subjectUrl }, imageUrl: { url: subjectUrl } })),
...
serve(async (req) => {
  ...
  try {
    ...
    if (!userAiConnectionId) {
      return new Response(JSON.stringify({ error: "Selecione uma conexão de imagem (OpenRouter) no builder antes de gerar." }), { status: 200, headers: jsonHeaders });
    }
