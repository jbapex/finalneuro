...
/** Refinamento com retângulo: o prefixo global pedia “layout estável”, o que anula “aumente”, “mude cor”, etc. */
...
/** Um único passe de “restauro”: a imagem atual é ao mesmo tempo referência e alvo (não há cópia pixel-perfect garantida). */
...
serve(async (req) => {
  ...
  try {
    ...
    const isRegionalEdit = !!(region && selectionAction) && !improveQualityMode;
    const hasSelectionAction = isRegionalEdit;
    const hasAnyAction =
      improveQualityMode ||
      hasSelectionAction ||
      instructionTrimmed ||
